...
public class Ventas extends javax.swing.JInternalFrame {
        ...
        try {
            ...
            while (repuestos.next()) {
                if (repuestos.getInt("respuestoIsActive") == 1) {
                    jCmbProducto.addItem(new Repuestos_LC(repuestos.getInt("idrepuesto"), repuestos.getString("namerepuesto")));
                }
            }
    ...
    private void jBtnAgregarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtnAgregarActionPerformed
        ...
        Repuestos_LC item = (Repuestos_LC) jCmbProducto.getSelectedItem();
    ...
    private void jCmbProductoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCmbProductoActionPerformed
        ...
        try {
            Repuestos_LC item = (Repuestos_LC) jCmbProducto.getSelectedItem();
            Repuestos_DAO repDao = new Repuestos_DAO();
            ResultSet rs = repDao.getRepuestos(item.getIdRepuesto());
            if (rs.next()) {
    ...
    private void jBtnFacturaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBtnFacturaActionPerformed
        try {
            ...
            if (factura > 0)
            {
                for (int x = 0; x < jTableListado.getRowCount(); x++)
                {
                    Repuestos_LC item = (Repuestos_LC)model.getValueAt(x, 0);
                    cantidad = (int)model.getValueAt(x, 1);
                    precioUnitario = (double)model.getValueAt(x, 2);
                    dTotal += (double)model.getValueAt(x, 3);
                    factura=facturaDAO.addDetalleFactura(facturaDAO.getLastFacturaID(), item.getIdRepuesto(), cantidad, precioUnitario, dTotal);
                    if (factura > 0)
                        bandera = true;
                    else
                        bandera = false;
    ...
    private javax.swing.JComboBox<Repuestos_LC> jCmbProducto;
