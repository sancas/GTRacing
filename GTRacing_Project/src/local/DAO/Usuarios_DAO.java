...
public class Usuarios_DAO {
    ...
    public int addUser(String username, String passwd, int empleado, int rol) {
        ...
        try {
            ...
            myPstat = myConn.con.prepareStatement("INSERT INTO public.usuarios(username, "
                    + "userpasswd, idempleado, idrol) VALUES (?, ?, ?, ?)");
    ...
    public int updateUser(int iduser, String username, String passwd, int empleado, int rol) {
        ...
        try {
            myConn.EstablecerConn();
            myPstat = myConn.con.prepareStatement("UPDATE public.usuarios SET username=?, "
                    + "userpasswd=?, idempleado=?, idrol=? WHERE idusuario=?");
            myPstat.setString(1, username);
            myPstat.setString(2, passwd);
            myPstat.setInt(3, empleado);
            myPstat.setInt(4, rol);
    ...
    public ResultSet getListUser() {
        try {
            myConn.EstablecerConn();
            myPstat = myConn.con.prepareStatement("select usuarios.idusuario, usuarios.username, "
                    + "usuarios.userpasswd, empleados.dui, roles.namerol from usuarios "
